...
export async function createMarket(input: CreateMarketInput): Promise<Result<{ marketId: string, txSignature: string }, string>> {
    ...
    try {
        ...
        const txSignature = await connection.sendTransaction(tx1, [keypair, ...info.vaultSigners], { maxRetries: 20 })
        ...
        const txSignature2 = await connection.sendTransaction(tx2, [keypair, ...info.marketSigners], { maxRetries: 20, skipPreflight: true })
        console.log("awaiting create market tx")
        await connection.confirmTransaction(txSignature2)
...
export async function createAndBuy(input: CreateAndBuy): Promise<Result<{
    ...
    buyTx.sign([keypair, ...buyFromPoolTxInfo.signers])
    ...
    return { Err: "maybe Failed to send the bundle" }
}
